...
/// A real cubit submitting an order through the [ApiClient].
///
/// A single [submit] triggers the dio 403 → refresh → retry interceptor, so
/// against a `403, 200` sequence the app records two POSTs and succeeds.
class OrdersCubit extends Cubit<OrdersState> {
  /// Creates an [OrdersCubit] over [apiClient].
  OrdersCubit(this.apiClient) : super(const OrdersState());

  /// The API client (the SUT).
